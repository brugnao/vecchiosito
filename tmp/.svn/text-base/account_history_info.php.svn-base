...
<?php
  ...
  for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
  	...
  	$product_query=tep_db_query("select * from " .TABLE_PRODUCTS." where products_id='$products_id' and products_status");
  	if ($product=tep_db_fetch_array($product_query)){
  		$product_link=tep_href_link(FILENAME_PRODUCT_INFO,'products_id='.$products_id);
    ...
    echo '          <tr>' . "\n" .
         ...
         '            <td class="main" valign="top">' . $product_link . $order->products[$i]['name'];
    if ($product_link!='')
    	echo '</a>';
...
//    echo '            <td class="main" align="right" valign="top">' . $currencies->format(tep_add_tax($order->products[$i]['final_price'], $order->products[$i]['tax']) * $order->products[$i]['qty'], true, $order->info['currency'], $order->info['currency_value']) . '</td>' . "\n" .
    echo '            <td class="main" align="right" valign="top">' . $order->products[$i]['pws_price_resume']['html'].'</td>' . "\n" .
         '          </tr>' . "\n";
//PWS eof
  }
?>
