    ...
    while ($reviews = tep_db_fetch_array($reviews_query)) {
    	...
    	$checkMultiImage = tep_db_query("select * from ".TABLE_PWS_PLUGINS." where plugin_code = 'pws_products_images'");
				   if(tep_db_num_rows($checkMultiImage) >= '1')  // è installato il plugin multimage?
			            {
		            	$image_query_vetrina=tep_db_query("select * from ".TABLE_PWS_PRODUCTS_IMAGES." where products_id='" . $reviews['products_id'] . "' order by sort_order");
							
			            	if (tep_db_num_rows($image_query_vetrina) >= '1') // modifico il primo lightbox (testo) se ci sono multi image
			            		$lc_text_reviews = '<a href="' . DIR_WS_IMAGES . $reviews['products_image'] . '" rel="lightbox[' . 'rw' . $reviews['products_id']. ']"  title="Click on the left/right side of image" target="_blank">' . $GLOBALS['pws_html']->getHtmlProductsImage($reviews['products_id'], $reviews['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';
							else 	
			            		$lc_text_reviews = '<a href="' . DIR_WS_IMAGES . $reviews['products_image'] . '" rel="lightbox[' . 'rw' .$reviews['products_id']. ']"   target="_blank">' . $GLOBALS['pws_html']->getHtmlProductsImage($reviews['products_id'], $reviews['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';
			            	
			            	
			            	while ($image_array = tep_db_fetch_array($image_query_vetrina))
							{
								$lc_text_reviews .= '<a href="' . DIR_WS_IMAGES . $image_array['products_image'] . '" rel="lightbox[' .'rw' . $reviews['products_id']. ']"  title="Click on the left/right side of image" target="_blank"></a>';			
							}
			            }
			        else           
			        	$lc_text_reviews = '<a href="' . DIR_WS_IMAGES . $reviews['products_image'] . '" rel="lightbox[' .'rw' . $reviews['products_id']. ']" target="_blank">' . $GLOBALS['pws_html']->getHtmlProductsImage($reviews['products_id'], $reviews['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';
...
?>
          <tr>
            <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr>
                <td class="main"><?php  echo '<a href="' . tep_href_link(FILENAME_PRODUCT_REVIEWS_INFO, 'products_id=' . $reviews['products_id'] . '&reviews_id=' . $reviews['reviews_id']) . '"><u><b>' . $reviews['products_name'] . '</b></u></a> <span class="smallText">' . sprintf(TEXT_REVIEW_BY, tep_output_string_protected($reviews['customers_name'])) . '</span>';
				// echo $lc_text_reviews;
				?></td>
              ...
              <tr class="infoBoxContents">
                <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td width="<?php echo SMALL_IMAGE_WIDTH + 10; ?>" align="center" valign="top" class="main"><?php //  echo '<a href="' . tep_href_link(FILENAME_PRODUCT_REVIEWS_INFO, 'products_id=' . $reviews['products_id'] . '&reviews_id=' . $reviews['reviews_id']) . '">' . $GLOBALS['pws_html']->getHtmlProductsImage($reviews['products_id'], $reviews['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>'; 
								echo $lc_text_reviews?></td>
