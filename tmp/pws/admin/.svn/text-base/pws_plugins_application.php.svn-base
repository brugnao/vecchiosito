<?php
/*
 * @filename:	pws_plugins_application.php
 * @version:	1.00
 * @project:	PWS
 *
 * @author:		Riccardo Roscilli <info@oscommerce.it>
 * @created:	12/giu/08
 * @modified:	12/giu/08 11:40:10
 *
 ...
 */
	...
	require_once DIR_FS_PWS_LANGUAGE.'language_pws_plugins_application.php';
	
	$plugin_type = 'application';
	$plugin_directory = DIR_FS_PWS_PLUGINS_APPLICATION;
	$plugin_key = 'PLUGIN_PRICES_INSTALLED';
	define('HEADING_TITLE', HEADING_TITLE_PLUGINS_APPLICATION);
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
				...
				<td class="dataTableHeadingContent" align="right">Ver.</td>
...
<?php
  ...
  for ($i=0, $n=sizeof($directory_array); $i<$n; $i++) {
    ...
    if ($installed=$pws_engine->isInstalledPlugin($class,$plugin_type))	{
    ...
    if ($installed && !$plin->plugin_removable && !$plin->plugin_configurable){
    	continue;
    }
    if (is_null($plugin)){
    	if (NULL==$plugin_code){
      ...
      if ($plin->plugin_code==$plugin->plugin_code) {
        if ($plugin->check() > 0) {
          ?> 
          <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href='<?=tep_href_link(FILENAME_PLUGINS, 'set=' . $set . '&plugin_code=' . $class . '&action=edit')?>'">
		  <?
        } else {
          ?> 
         ...
         ?> 
		  <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href='<?=tep_href_link(FILENAME_PLUGINS, 'set=' . $set . '&plugin_code=' . $class) ?>'">
	     <?
      }
?>
                ...
                <td class="dataTableContent" align="right"><?=$plin->plugin_version_const?></td>                
