...
// query standard
$product_select = 'pd.language_id, p.products_id, pd.products_name, pd.products_description, p.products_model, p.products_quantity, p.products_image, p.products_weight, pd.products_url, pd.products_youtube_url, p.products_price, p.products_tax_class_id,
	p.products_date_added, p.products_date_available, p.manufacturers_id ,p.EAN, p.link';

//modulo esprinet
if (file_exists(DIR_FS_CATALOG . 'distributors/config/esprinet.xml'))
$product_select  .= ' , p.Codice';
...
<link rel="stylesheet" href="lightbox.css" type="text/css"
	media="screen" />
<?=$pws_engine->triggerHook('CATALOG_PRODUCT_INFO_HEAD')?>

<script language="javascript"><!--
<?=$pws_prices->catalogProductInfoJavascript($HTTP_GET_VARS['products_id'])?>
<?=$pws_prices->catalogJavascript()?>
<?=$pws_engine->triggerHook('CATALOG_PRODUCT_INFO_JAVASCRIPT')?>
//--></script>
<?php
// ICECAT {{
	...
	<tr>
	  ...
	  <table border="0" width="50%" cellspacing="0" cellpadding="2px">';
					...
					<tr>
						...
						<table width="100%" border="0" cellspacing="2" cellpadding="2"
							...
							//	if (ENABLE_PRODUCT_SHIPPING_COST == 'true') $colspan = '2' ?>
								<td class="productsPrice"><?php  echo $products_price; ?></td>
							</tr>
							<tr>
								<!--  costi di spedizone -->
							<?php if(ENABLE_PRODUCT_SHIPPING_COST == 'true') {
								...
								<table width="100%" border="0" cellspacing="2" cellpadding="2"
									...
									<tr>
										...
										<td class="main" valign="middle" align="right"><?php 
						...
						<?php if($GLOBALS['pws_prices']->displayPrices())
						{
							?>
						<table width="100%" border="0" cellspacing="2" cellpadding="2"
		    ...
		    // visualizza la scheda esprinet
						...
						{
							// controlla se il file in locale esiste o no altrimenti lo preleva dal sito remoto
							//  il nuovo file ha come nome il codice esprinet.html percorso /cache
							// $localLink = "/var/www/vhosts/test.bestdigit.it/httpdocs/cache/" . $products_id . ".html";
							 $localLink =   $products_id . '.html';
							// $localLink = 'cache/' . $localLink;
							if (file_exists("/var/www/vhosts/test.bestdigit.it/httpdocs/cache/" . $localLink))
							{
							$distLink = "/var/www/vhosts/test.bestdigit.it/httpdocs/cache/" . $localLink;
							include $distLink;
							}
							else
							{
							...
							//	print $file =  $rfR->getContent();
							//	exit;
								/*$fc = fopen("cache/" . $localLink, "w");
								//while (!feof ($file)) {
									//$line = fread ($file, 1028);
									fwrite($fc,$rfR->getContent());
							//	}
								fclose($fc);
								*/
							//	$distLink = "cache/" . $localLink;
							include ("esprinet_viewer.php");
							?>
							
							<iframe src="esprinet_viewer.php" width="100%"
