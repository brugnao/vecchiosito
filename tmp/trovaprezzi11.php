...
//   include('includes/configure.php');        // include i dati per la connessione al DB
  require 'includes/application_top.php'; 
   ...
   IF(p.products_quantity - p.products_ordered > 0, 'disponibile','non disponibile') as availability,
   ...
   if($result = tep_db_query( $listing_sql/*, DbConnection()*/ ) ){
      ...
      while($row=tep_db_fetch_array($result)){
         ...
         /*$tablestring.="<tr>". // Scommentare per comporre una tabella con i dettagli dei prodotti utile per il debug
               ...
               "<td>" . $shippingprice . "</td>"  .
                 "</tr>"; */
         $filestring.=$row["products_id"]."|".$row["products_name"]."|".$site_url."/images/".$row["products_image"]."|".$site_url."/product_info.php?products_id=".$row["products_id"]."&from=TROVAPREZZI|".$cat_list."|".$final_price."|".$descrizione1."|".$row["marca"]."|" . $row["availability"] ."|" . $shippingprice ."|" . $row["codprod"] ."<endrecord>\r\n";
         $rescount += 1;// $rescount++;
      }  
      echo $filestring;
   }else{
