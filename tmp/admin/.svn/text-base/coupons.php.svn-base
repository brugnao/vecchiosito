  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'deleteconfirm':
		...
		// esclusione per i gruppi
		tep_db_query($sql = "delete from " . TABLE_DISCOUNT_COUPONS_TO_CUSTOMER_GROUPS . " where coupons_id = '" .$coupons_id. "'");
        //end exclusions
      ...
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td class="specialPrice" align="right">NOTICE: <a href="<?php echo tep_href_link( DIR_WS_LANGUAGES.$language.'/'.FILENAME_DISCOUNT_COUPONS_MANUAL ) ?>"><? echo HEADING_TITLE_VIEW_MANUAL ?></a></td>
          </tr>
        </table></td>
      </tr>
<?php
      ...
      if (is_object($cInfo)) {
        ...
        $contents[] = array('align' => 'center',
        										'text' => '<a href="' . tep_href_link(FILENAME_DISCOUNT_COUPONS, 'page=' . $HTTP_GET_VARS['page'] . '&cID=' . $cInfo->coupons_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> '.
        															...
        															.'<a href="' . tep_href_link(FILENAME_DISCOUNT_COUPONS_EXCLUSIONS, 'cID='.$cInfo->coupons_id.'&type=customer_groups') . '">' . tep_image_button('button_customer_groups_excl.gif', IMAGE_CUSTOMER_GROUPS_EXCLUSIONS, 'hspace="2" vspace="2"') . '</a>'
