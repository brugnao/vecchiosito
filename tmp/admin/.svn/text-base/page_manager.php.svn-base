...
switch ($action) {
    ...
    case 'update':
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
            ...
            $title_field_name=tep_db_prepare_input($HTTP_POST_VARS['pages_title'][$language_id]);
        ...
        if ($page_error == false) {
            ...
            for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
                $language_id = $languages[$i]['id'];
                $sql_data_array_pages = array('sort_order' => tep_db_prepare_input($HTTP_POST_VARS['sort_order']),
                                              'status' => '1',
                                              'page_type'   => $page_type);

                $sql_data_array_pages_description = array('pages_title' => tep_db_prepare_input($HTTP_POST_VARS['pages_title'][$language_id]),
                                                          'pages_html_text' => tep_db_prepare_input($HTTP_POST_VARS['pages_html_text'][$language_id]),
                                                          'intorext'   => tep_db_prepare_input($HTTP_POST_VARS['intorext'][$language_id]),
                                                          'externallink' => tep_db_prepare_input($HTTP_POST_VARS['externallink'][$language_id]),
                                                          'link_target' => tep_db_prepare_input($HTTP_POST_VARS['link_target'][$language_id]));
                if ($action == 'insert') {
                    $bID="";
                    if ($i == 0)  {
                    ...
                    $selectexists=tep_db_query("select count( * ) as `countrecords` from `".TABLE_PAGES_DESCRIPTION."` where pages_id='" . (int)$pages_id . "' and language_id='". $language_id ."'");
                    $recordexists = tep_db_fetch_array($selectexists);
                    if($recordexists['countrecords'] >= 1 )  {
                        tep_db_perform(TABLE_PAGES_DESCRIPTION, $sql_data_array_pages_description, 'update', "pages_id = '" . (int)$pages_id . "' and language_id='".$language_id."'");
                    } else  {
                        $pageid_merge= array('pages_id' => $pages_id,
                                             'language_id' => $language_id);
...
if ($action == 'new') {
    ...
    if (isset($HTTP_GET_VARS['bID'])) {
        ...
        $page_query = tep_db_query("select s.status, s.sort_order, s.page_type, p.pages_title, p.pages_html_text, p.intorext, p.externallink, p.link_target, p.language_id from " 
                . TABLE_PAGES . " s left join " . TABLE_PAGES_DESCRIPTION . " p on s.pages_id=p.pages_id
                where s.pages_id = '" . (int)$bID . "'");
...
?>
 ...
 <tr>
            ...
            <td class="main"><?echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;'?><select name="page_type" size=1>
                                 ...
                                 <option value="4" <? if($page_type == 4) { echo "selected"; } ?>>Terms And Conditions</option>
                                 </select></td>
          </tr>
...
<?php 
        echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']). '&nbsp;'.tep_draw_input_field('pages_title['.$languages[$i]['id'] . ']', $pagetitle[$languages[$i]['id']], '', true); 
    }
?>
</td>
          </tr>
...
<?
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        if(empty($intorext[$languages[$i]['id']]) or $intorext[$languages[$i]['id']] == "0")  {
...
?>
 <tr>
            ...
            <td class="main">
            <? echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) ?><input type="radio" value="0" name="intorext[<?php echo $languages[$i]['id']; ?>]" <?php echo $internalchecked; ?> onClick='disableIt("<? echo 'externallink['.$languages[$i]['id'] . ']'; ?>");'><? echo TEXT_TARGET_INTERNAL; ?>
            &nbsp;&nbsp;
            <? echo tep_draw_separator('pixel_trans.gif', '24', '15'); ?><input type="radio" value="1" name="intorext[<?php echo $languages[$i]['id']; ?>]" onClick='enableIt("<? echo 'externallink['.$languages[$i]['id'] . ']'; ?>");' <?php echo $externalchecked;?> ><? echo TEXT_TARGET_EXTERNAL; ?>
</td>
          </tr>
  <tr>
            <td class="main"><?php echo TEXT_PAGES_EXTERNAL_LINK; ?></td>
            <td class="main"><?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']).'&nbsp;'.tep_draw_input_field('externallink['.$languages[$i]['id'] . ']', $externallink[$languages[$i]['id']], 'id='.'"externallink['.$languages[$i]['id'].']"'. $disabledlinkbox, false); ?></td>
          </tr>
...
?>
  <tr>
            ...
            <td class="main">
<?php echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']);?><input type="radio" value="0" name="link_target[<?php echo $languages[$i]['id']; ?>]" <?php echo $samewindowchecked; ?>><? echo TEXT_TARGET_SAMEWINDOW; ?>
<input type="radio" value="1" name="link_target[<?php echo $languages[$i]['id']; ?>]" <?php echo $newwindowchecked; ?>><? echo TEXT_TARGET_NEWWINDOW; ?>
</td>
          </tr>
<tr>
...
<?php for ($i = 0, $n = sizeof($languages); $i < $n; $i++) { ?>
            ...
            tep_draw_fckeditor('pages_html_text['.$languages[$i]['id'] . ']', '500', '300', $pages_html_text[$languages[$i]['id']]); 
            // tep_draw_textarea_field('pages_html_text['.$languages[$i]['id'] . ']', 'soft', '80', '20', $pages_html_text[$languages[$i]['id']]); ?>
<?php } ?>
</td>
