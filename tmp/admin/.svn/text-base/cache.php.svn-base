...
*/
  ...
  if (tep_not_null($action)) {
  	if ($action == 'reset' && $HTTP_GET_VARS['block'] == 'Turbocache')
  	{
		$filesDaCancellare = glob('../temp/*turbo*');
...
<?php
  ...
  // and now turbo cache !!
  $cache_title = 'Turbocache';
  if(file_exists('../temp/index.php_turbo__0____________.html'))
  $cache_mtime = strftime(DATE_TIME_FORMAT, filemtime('../temp/index.php_turbo__0____________.html')) ;
  else
   $cache_mtime = TEXT_FILE_DOES_NOT_EXIST;
 ?>
             <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)">
                <td class="dataTableContent"><?php echo  $cache_title; ?></td>
                <td class="dataTableContent" align="right"><?php echo $cache_mtime; ?></td>
                <td class="dataTableContent" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_CACHE, 'action=reset&block=' . $cache_title, 'NONSSL') . '">' . tep_image(DIR_WS_IMAGES . 'icon_reset.gif', 'Reset', 13, 13) . '</a>'; ?>&nbsp;</td>
              </tr>
              <tr>
