...
if (!function_exists('dont_set')){
	function dont_set($value,$key=NULL){
	  	$output=$value;
	  	if (is_null($key)){
...
//PWS bof
      case 'prices':
      	require DIR_FS_PWS_ADMIN.'pws_plugins.php';
      	exit;
//      	$module_type = 'prices';
//        $module_directory = DIR_FS_PWS_PLUGINS_PRICES;
//        $module_key = 'MODULE_PRICES_INSTALLED';
//        define('HEADING_TITLE', HEADING_TITLE_MODULES_PRICES);
      	break;
      case 'application':
      	require DIR_FS_PWS_ADMIN.'pws_plugins_application.php';
      	exit;
//PWS eof
      case 'payment':
      default:
...
<?php
  ...
  for ($i=0, $n=sizeof($directory_array); $i<$n; $i++) {
    ...
    if (tep_class_exists($class)) {
      $module = new $class;
      if ($module->check() > 0) {
	    $sort_order=floatval($module->sort_order);
       	while (isset($installed_modules[strval($sort_order)])){
       		$sort_order+=0.1;
       	}
        $installed_modules[strval($sort_order)] = $file;
      }
...
<?php
  ...
  $resort_modules=array();
  reset($installed_modules);
  foreach ($installed_modules as $key=>$module){
  	$val=floatval($key);
  	$deckey=(int)($val/10);
  	$key=chr(ord('a')+$deckey).$key;
  	$resort_modules[$key]=$module;
  }
  $installed_modules=$resort_modules;
  ksort($installed_modules,SORT_STRING);
