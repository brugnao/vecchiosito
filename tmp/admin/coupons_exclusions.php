 ...
 */
  ...
  $display_fields = array(
						  ...
						  'customer_groups' => array ('cg.customers_group_id' => true,
												 'cg.customers_group_name' => true),	
  
                          'manufacturers' => array( 'm.manufacturers_id' => false,
                                                    //'mi.manufacturers_url' => false,
                                                    'm.manufacturers_name' => true ),
                                                    
                          'zones' => array( 'gz.zone_name' => true //,
                                           // 'gz.zone_description' => true 
                                           ),  
    ...
    $display_fields = array_keys( $display_fields[$type], true );
  	...
  	switch( $type ) {
		...
		case 'customer_groups':
			$sql_selected = 'SELECT dc2cg.customers_group_id AS id'.$display_fields.'
			FROM '.TABLE_DISCOUNT_COUPONS_TO_CUSTOMER_GROUPS.' dc2cg
			LEFT JOIN '.TABLE_CUSTOMERS_GROUPS.' cg
				ON cg.customers_group_id=dc2cg.customers_group_id
			WHERE dc2cg.coupons_id="'.$coupons_id.'"';
			$sql_all = 'SELECT cg.customers_group_id AS id'.$display_fields.'
			FROM '.TABLE_CUSTOMERS_GROUPS.' cg
			%s';
			$where = 'WHERE cg.customers_group_id NOT IN( %s) ';
		break;		
      ...
      case 'zones' :
 /*       $sql_selected = 'SELECT dc2z.geo_zone_id AS zone_id'.$display_fields.'
                          FROM '.TABLE_DISCOUNT_COUPONS_TO_ZONES.' dc2z
                          LEFT JOIN '.TABLE_ZONES.' gz
                            USING( gz.zone_id )
                          WHERE dc2z.coupons_id="'.$coupons_id.'"';
*/
        $sql_selected = 'SELECT dc2z.geo_zone_id AS id, gz.zone_id, gz.zone_name
							FROM discount_coupons_to_zones dc2z, zones gz
					WHERE dc2z.geo_zone_id = gz.zone_id
					AND dc2z.coupons_id="'.$coupons_id.'"';
			
        $sql_all = 'SELECT gz.zone_id AS id'.$display_fields.'
                    FROM '.TABLE_ZONES.' gz
                    %s';
        $where = 'WHERE gz.zone_id NOT IN(%s) ';
      break;
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="5" cellpadding="0">
          <tr>
            ...
            <td class="specialPrice" align="right">Nota: <a href="<?php echo tep_href_link( DIR_WS_LANGUAGES.$language.'/'.FILENAME_DISCOUNT_COUPONS_MANUAL ).'">'.HEADING_TITLE_VIEW_MANUAL; ?></a></td>
          </tr>
          <tr>
          	<td colspan="2">
<?php
