...
$tn_path = DIR_WS_IMAGES.'imagecache/'; // The default is 'thumbnails/', should be chmod 777
//
...
$tn_browser_cache = false; // The default is true
//
...
//_GD_start
// Controlla l'esistenza del path per i thumbnails
...
function modify_tn_path($file)
{
	...
	str_replace('\\','/',$file);
	$file = substr($file, strrpos($file, $pathSep)+1);
	return $tn_path.$file;
...
/*
	...
	*/
	...
	if ($image[2] == 2 || ($image[2] == 1 && $gif_as_jpeg))
	{
		if ($tn_server_cache)
		{
			$thumbnail = modify_tn_path($_GET['img']);
			imagejpeg($tmp_img,$thumbnail, 100);
			http_headers($thumbnail);
		}
		else
