...
*/
	...
	$skin->set('box_heading',sprintf(TABLE_HEADING_SPECIALS, strftime('%B')));
	if (!isset($cPath) && isset($_REQUEST['cPath']))
		$cPath=$_REQUEST['cPath'];
	if(RANDOMIZE == 'on')  $new_products_query	= tep_db_query("select p.products_id, pd.products_name, p.products_price, p.products_tax_class_id, p.products_image, s.specials_new_products_price 
		from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_SPECIALS . " s where p.products_status = '1' and p.products_id = s.products_id and pd.products_id = s.products_id and pd.language_id = '" . (int)$languages_id . "' and s.status = '1' and s.customers_group_id = '0'	order by rand() desc	limit " . MAX_DISPLAY_SPECIAL_PRODUCTS);
	else $new_products_query	= tep_db_query("select p.products_id, p.products_date_added, pd.products_name, p.products_price, p.products_tax_class_id, p.products_image, s.specials_new_products_price 
		from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_SPECIALS . " s where p.products_status = '1' and p.products_id = s.products_id and pd.products_id = s.products_id and pd.language_id = '" . (int)$languages_id . "' and s.status = '1' and s.customers_group_id = '0'	order by  p.products_date_added desc	limit " . MAX_DISPLAY_SPECIAL_PRODUCTS);
	...
	if (tep_db_num_rows($new_products_query)){
		while ($new_products = tep_db_fetch_array($new_products_query)) {
			...
			$product['products_name'] = tep_get_products_name($products_id);
				...
				$checkMultiImage = tep_db_query("select * from ".TABLE_PWS_PLUGINS." where plugin_code = 'pws_products_images'");
				   if(tep_db_num_rows($checkMultiImage) >= '1')  // è installato il plugin multimage?
			           ...
			           //		$lc_text = '<a href="' . DIR_WS_IMAGES . $new_products['products_image'] . '" rel="lightbox[' .$new_products['products_id']. ']"  title="Click on the left/right side of image" target="_blank">' . $GLOBALS['pws_html']->getHtmlProductsImage($new_products['products_id'], $new_products['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';
							    $lc_text = tep_output_image($new_products['products_id'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, ' rel="lightbox[' .$new_products['products_id']. ']" title="Click on the left/right side of image" target="_blank"' );
			            	
			            	else 	
							    $lc_text = tep_output_image($new_products['products_id'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, ' rel="lightbox[' .$new_products['products_id']. ']" target="_blank"' );
			            				            	
			            	
			            	while ($image_array = tep_db_fetch_array($image_query))
							{
								$lc_text .= '<a href="' . DIR_WS_IMAGES . $image_array['products_image'] . '" rel="lightbox[' . $new_products['products_id']. ']"  title="Click on the left/right side of image" target="_blank"></a>';			
							}
			            }
			        else           
							    $lc_text = tep_output_image($new_products['products_id'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, ' rel="lightbox[' .$new_products['products_id']. ']" target="_blank"' );
