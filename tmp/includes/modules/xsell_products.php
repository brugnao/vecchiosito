...
<?php
      ...
      while ($xsell = tep_db_fetch_array($xsell_query)) {
        ...
        $info_box_contents[$row][$col] = array('align' => 'center',
                                               ...
                                               'text' => '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $xsell['products_id']) . '">' 
        . $GLOBALS['pws_html']->getHtmlProductsImage( $xsell['products_id'], $xsell['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a><br><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $xsell['products_id']) . '">' . $xsell['products_name'] . '</a>');
        $col ++;
        if ($col > 2) {
          $col = 0;
          $row ++;
