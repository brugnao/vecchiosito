...
*/
	...
	//	$skin->set('image',$GLOBALS['pws_html']->getProductsImage($random_product['products_id'],SMALL_IMAGE_WIDTH,SMALL_IMAGE_HEIGHT));
		...
		// exit;
		$expires_date = str_replace("-",",",$random_product[expires_date]);
		$expires_date = str_replace(":",",",$expires_date);
		$expires_date = str_replace(" ",",",$expires_date);
		$skin->set('dummyimg','<img  src="/images/pixel_trans.gif" onload="countdown(' . $expires_date  . ')" />');
		echo $skin->execute();
