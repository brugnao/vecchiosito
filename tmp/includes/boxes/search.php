...
*/
	...
	$skin->set('form', array(
		...
		,'id'=>'quick_find'
		,'method'=>'get'
		,'action'=>	tep_href_link(FILENAME_ADVANCED_SEARCH_RESULT, '', 'NONSSL', false)
		)
	);
	...
	$skin->set('submit_image',tep_image_submit('button_quick_find.jpg', BOX_HEADING_SEARCH, 'align="absbottom"'));
//	$skin->set('submit_image',tep_image_submit('button_quick_find.jpg', BOX_HEADING_SEARCH));
	
	$skin->set('search_script',
	'
...
var phpHelpUrl="'.tep_href_link(FILENAME_PRODUCT_INFO,'products_id=','NONSSL',false,false).'";
...
function addToCache(keyword, values, urls)
{
...
function checkCache(keyword)
{
  ...
  for(i=keyword.length-2; i>=0; i--)
  {
    ...
    {            
	  ...
	  tmp_i = keywordResultsSize;
          keywordResults[tmp_i] = cacheResults[j];
	  urlkeywordResults[tmp_i] = urlResults[j];
          keywordResultsSize++;
        }
      }      
      // add all the keyword\'s prefix results to the cache
      addToCache(keyword, keywordResults,urlkeywordResults) ;      
      return true;  
    }
  }
  // no match found
...
{  
  ...
  if(keyword != "" && !isKeyUpDownPressed)
  {
    ...
    {   
      ...
      displayResults(keyword, oCache[keyword], urlCache[keyword]);                          
    }
...
function updateSuggestions()
{
  ...
  else
  {
    ...
    addToCache(httpRequestKeyword, nameArray, urlArray);              
  }
}
...
{  
  ...
  if(!oCache[keyword] && keyword) {
    addToCache(keyword, results_array, url_array);
  }
...
function updateKeywordValue(oTr)
{
  ...
  var crtLink = oCache[userKeyword][0];
...
/* function that handles the mouse entering over a suggestio\'s area 
   event */
