 ...
 */
 ...
 function get_coupon( $code ) {
		//BOF SPPC to get customers group id
...
if( tep_db_num_rows( $check_user_query ) > 0 ) {
          ...
          //use this to debug exclusions:
				...
				if( tep_db_num_rows( $check_user_query ) > 0 ) {
					$this->message( ENTRY_DISCOUNT_COUPON_ERROR ); //display the error message
          //use this to debug exclusions:
					//$this->message( 'Customer groups exclusion check failed' );
				}
				//end customer_group_exclusions			
	        
				
		//shipping zone exclusions

	
	     if(isset($sendto) )     // patch per gestire le esclusioni per zone solo dopo che ha scelto la destinazione
	     {
 			$delivery_query = tep_db_query("Select * from " . TABLE_ADDRESS_BOOK . " where customers_id = " . $customer_id . " and address_book_id = " . $sendto);
			$delivery = tep_db_fetch_array($delivery_query);
 			...
 			$check_user_query = tep_db_query($sql = 'SELECT dc2z.geo_zone_id
		                                                  FROM '.TABLE_DISCOUNT_COUPONS_TO_ZONES.' dc2z
		                                                   WHERE ( dc2z.geo_zone_id='.$delivery['entry_zone_id'].' )
    ...
    function check_coupons_min_order() {
     ...
     //     print "totale carrello coupons: " . $total;
     //     exit;
   ...
   //   if( MODULE_ORDER_TOTAL_DISCOUNT_COUPON_DISPLAY_SUBTOTAL == 'false' ) {
     ...
     // $shown_price = ( $this->apply_tax( $product['final_price'], $product['tax'] ) * $product['qty'] ) - ( $discount['applied_discount'] + $discount['discount_tax'] );
      ...
      $shown_price = (  $product['final_price'] * $product['qty'] ) - ( $discount['applied_discount'] + $discount['discount_tax'] );
