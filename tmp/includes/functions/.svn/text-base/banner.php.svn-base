...
// WebMakers.com Added: Banner Manager
        $banner = tep_random_select("select banners_id, banners_title, banners_image, banners_html_text, banners_open_new_windows, banners_on_ssl from " . TABLE_BANNERS . " where status = '1' " . $my_banner_filter . "  and banners_group = '" . $identifier . "'");
      	...
      	// modifica per la nuova gestione dei banner nel template sts. se non trova il banner restituisce '' in modo che non ci siano casini quanto il cliente disabilita un banner
...
// disabilitata la gestione di più zone per i banner
  return tep_random_select("select banners_id, banners_title, banners_image, banners_html_text, banners_open_new_windows from " . TABLE_BANNERS . " where status = '1'  " . $my_banner_filter . " ");
    } elseif ($action == 'static') {
