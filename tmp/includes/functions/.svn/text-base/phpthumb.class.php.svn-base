...
//ob_end_clean();
//
include_once(dirname(__FILE__).'/phpthumb.functions.php');
...
//		}
	...
	function RenderOutput2File($filename) {
		if (!$this->useRawIMoutput && !is_resource($this->gdimg_output)) {
			$this->DebugMessage('RenderOutput() failed because !is_resource($this->gdimg_output)', __FILE__, __LINE__);
			return false;
		}
		...
		switch ($this->thumbnailFormat) {
			case 'wbmp':
				...
				ImageJPEG($this->gdimg_output, $filename, $this->thumbnailQuality);
				$this->outputImageData = file_get_contents($filename);//ob_get_contents();
				break;
			...
			case 'png':
				...
				ImagePNG($this->gdimg_output,$filename);
				$this->outputImageData = file_get_contents($filename);//ob_get_contents();
				break;
			...
			case 'gif':
				...
				ImageGIF($this->gdimg_output,$filename);
				$this->outputImageData = file_get_contents($filename);//ob_get_contents();
				break;
	...
	function RenderToFile($filename) {
		...
		if ($this->RenderOutput2File($renderfilename)) {
