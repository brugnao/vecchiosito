...
<?php
  ...
  $skin->set('box_heading',BOX_HEADING_SHOPPING_CART);
  $skin->set('no_products',$cart->count_contents()==0);
  $skin->set('no_products_text',BOX_SHOPPING_CART_EMPTY);
  $products=$cart->get_products();
  for ($i=0, $n=sizeof($products); $i<$n; $i++) {
  	$products[$i]['href']=tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $products[$i]['id']) ;
  	$products[$i]['new_item']=$new_products_id_in_cart == $products[$i]['id'];
  }
  $skin->set('products',$products);
  $skin->set('products',   $cart->count_contents());
  $skin->set('cart_total',$currencies->format($cart->show_total()));
  echo $skin->execute();
